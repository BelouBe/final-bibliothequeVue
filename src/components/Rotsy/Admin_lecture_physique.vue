<template lang="">
  <div class="lecture_container">
    <div class="fondNoir" v-if="showSwall">
      <Admin_modification_physique
        v-if="rubrique === 'update'"
        @closeSwall="toggleSwall"
        :item="itemChoosen"
        @goToAccueil="goToAccueil"
      />
      <Admin_suppression_physique
        v-if="rubrique === 'delete'"
        @closeSwall="toggleSwall"
        :item="itemChoosen"
        @goToAccueil="goToAccueil"
      />
      ...
      <div class="liste_oeuvres">
        <ul class="liste">
          <li v-for="item in listes" :key="item.key" class="item-oeuvre">
            ...
            <div class="center">
              <img
                src="../../assets/Rotsy/Icon_update.png"
                alt="icon_update"
                @click="toggleRubrique(item, $event)"
              />
              <img
                src="../../assets/Rotsy/Icon_delete.png"
                alt="icon_delete"
                @click="toggleRubrique(item, $event)"
              />
            </div>
            <div class="right">
              <p>{{ item.dateDeParution }}</p>
              <p>{{ item.categorie }}</p>
            </div>
          </li>
        </ul>
...
export default {
  ...
  mounted() {
    fetch(config + "/api/bookPhysique")
      ...
      .then((responseJson) => {
        this.listes = responseJson.map((item) => {
          return {
            ...item,
            apropos: item.apropos,
            dateDeParution: item.dateDeParution.split("T")[0],
          };
        });
      });
  },
